(function(){
  ...
  app.controller('searchController', ['$scope', '$routeParams', 'searchService', 'idMappingService', function($scope, $routeParams, searchService, idMappingService) {
    ...
    var buildData = function(data, query) {
      ...
      queryValues = ['Akin', 'Austin', 'Texas'];

      for(var key in data['neighborhoods']) {
        result = '';
        cityName = $scope.cityIdToName[data['neighborhoods'][key]['city']];
        stateName = $scope.stateIdToName[data['neighborhoods'][key]['stateCode']];
        neighborhoodName = data['neighborhoods'][key]['name'];
    ...
    var init = function() {
      searchService.callAPI().then(function(data){buildData(data, query); $scope.rows = $scope.searchResults;}, function(data) {alert(data)});
    };
    init();
  ...
  app.controller('splashController',['$scope', function($scope){ 
    this.searchURL = function() {
      console.log($scope.selectedValue.name);
  ...
  app.service('idMappingService',['$q','$http', '$location', '$rootScope', function($q,$http,$location,$rootScope){
    ...
    var idToName = function(topLevelName, idTag, nameTag){
      this.cityIdToName = {};
      //update this later
      dataGetHelper('/~bjq59/vas/json_data/'+topLevelName+'.json').then(
        function(data){
          // console.log(data);
